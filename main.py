...
from common.networks import MLP_diff, MLP, En_MLP, Feature_Attn_MLP, En_Feature_Attn_MLP, RNN, RNN_AddAtte, RNN_Multihead_SelfAtte
...
def get_args():
    ...
    parser.add_argument('--net_type', type=str, default='LSTM_Multihead_SelfAtte',
                        choices=['MLP', 'diffusion', 'En_MLP', 'Feature_Attn_MLP', 'En_Feature_Attn_MLP', 'GRU', 'LSTM',
    ...
    parser.add_argument('--num_episode', type=int, default=8000)           # 1000, 5000
    parser.add_argument('--batch_size', type=int, default=512)
    parser.add_argument('--wd', type=float, default=0)                    # 1e-4
    parser.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    parser.add_argument('--actor-lr', type=float, default=5e-7)      # 1e-3, 3e-7
    ...
    parser.add_argument('--epsilon', type=float, default=1.0)               # 选择动作时epsilon-greedy的参数
    parser.add_argument('--epsilon_decay', type=float, default=0.996)         # 每轮训练后epsilon衰减率
    parser.add_argument('--frame_size', type=float, default=1000)            # 帧大小（KB）       【几百？】
    ...
    parser.add_argument('--r_T', type=float, default=100)                     # 车辆计算结束时刻超过离开时刻的reward惩罚项系数
    parser.add_argument('--r_S', type=float, default=100)                     # 车辆直至到达路口都没有执行计算的reward惩罚项
    parser.add_argument('--lamda', type=float, default=0.03)               # 目标函数比例系数 （误差 + lamda * 传输量）
    parser.add_argument('--warm_up_vehicles', type=float, default=35)   # 预热车辆数量，这些车为仿真初始时段的车辆，环境不稳定，轨迹不计入训练
    ...
    parser.add_argument('--v_max', type=float, default=24)
    ...
    parser.add_argument('--a', type=float, default=0.05)                   # 计算车辆紧迫性权重的参数
...
if __name__ == '__main__':
    ...
    check_dir = f"checkpoints/{datetime.now().strftime('%Y%m%d_%H%M%S')}"
